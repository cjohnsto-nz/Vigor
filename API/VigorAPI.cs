...
namespace Vigor.API
{
    ...
    public class VigorAPI : IVigorAPI
    {
        ...
        public StaminaActionType RegisterActionType(string actionId, string displayName)
        {
            ...
            {
                var modSystemActions = _api.ModLoader.GetModSystem<VigorModSystem>();
                _api.Logger.Warning($"[{modSystemActions.ModId}] Action type {actionId} already registered by {existingAction.ModId}");
        ...
        public StaminaModifier RegisterModifier(string modifierId, string displayName, System.Func<EntityPlayer, string, float, float> calculationDelegate)
        {
            ...
            {
                var modSystemModifiers = _api.ModLoader.GetModSystem<VigorModSystem>();
                _api.Logger.Warning($"[{modSystemModifiers.ModId}] Modifier {modifierId} already registered by {existingModifier.ModId}");
        ...
        public bool ConsumeStamina(string actionTypeId, float amount, EntityPlayer player, bool ignoreFatigue = false)
        {
            ...
            foreach (var modifier in _registeredModifiers.Values)
            {
                ...
                {
                    var vigorSystem = _api.ModLoader.GetModSystem<VigorModSystem>();
                    _api.Logger.Error($"[{vigorSystem.ModId}] Error applying modifier {modifier.ModifierId}: {ex.Message}");
                }
            }
